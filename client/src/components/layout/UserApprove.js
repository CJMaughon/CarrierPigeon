...
import { getUnapprovedUsers, approveUsers } from '../../actions/users';
...
const UserApprove = ({ getUnapprovedUsers, approveUsers, user: { users, loading } }) => {
    ...
    const handleSelectAllClick = event => {
        if (event.target.checked) {
            ...
            const selected_ids = users.map(n => n._id);
            setSelected(newSelecteds);
            setIdsSelected(selected_ids);
            return;
        }
        setSelected([]);
        setIdsSelected([]);
    ...
    const handleClick = (event, id, name) => {
        ...
        if (selectedIndex === -1) {
            ...
            newIdsSelected = newIdsSelected.concat(idsSelected, id);
        } else if (selectedIndex === 0) {
            newSelected = newSelected.concat(selected.slice(1));
            newIdsSelected = newIdsSelected.concat(idsSelected.slice(1));
        } else if (selectedIndex === selected.length - 1) {
            newSelected = newSelected.concat(selected.slice(0, -1));
            newIdsSelected = newIdsSelected.concat(idsSelected.slice(0, -1));
        } else if (selectedIndex > 0) {
    ...
    return loading ? (
        <Spinner />
            ...
            <Fragment>
                <Navbar />
                <h1 className='medium text-primary'>Approve Instructor</h1>
            ...
            <TableRow>
                <TableCell padding="checkbox">
                    <Checkbox
                        ...
                        inputProps={{ 'aria-label': 'select all instructors' }}
                    />
                </TableCell>
